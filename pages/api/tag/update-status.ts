    ...
    if (req.method === 'PUT') {
        try {
            ...
            return res.status(StatusCode.OK).json(transformResponse({
                data: updatedTag,
                message: 'Cập nhật trạng thái từ khóa thành công.',
                statusCode: StatusCode.OK,
            }));

        } catch (error) {
            console.error('Lỗi khi cập nhật trạng thái từ khóa:', error);
            return res.status(StatusCode.INTERNAL_SERVER_ERROR).json(transformResponse({
                data: null,
                message: 'Đã xảy ra lỗi khi cập nhật trạng thái từ khóa.',
